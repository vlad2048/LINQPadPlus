...
public class Fag
{
	...
	protected internal Fag(string name) => this.name = name;
	...
	// @formatter:off
	public Fag id(string id__) => this.With(() => Id = id__);
	public Fag cls(string className, bool condition = true) => this.With(() => classes.Add(className), condition);
	public Fag style(string style, bool condition = true) => this.With(() => styles.Add(style), condition);
	public Fag attr(string key, JSVal? value, bool condition = true) => this.With(() => attrs[key] = value, condition);
	// @formatter:on

	public Fag this[params object[] kids_] => this.With(() =>
	{
		if (t.VoidElements.Contains(name))
	...
	public Fag On(string evtName, string[] evtProps, Action<IDictionary<string, string>> action) =>
		this.With(() =>
	...
	public Fag On(string evtName, Action action) =>
		this.With(() =>
			whenPostRender.Subscribe(_ =>
			{
				var ctrl = new Control(name)
	...
	public Fag onReady(Action onReady) => this.With(() => whenPostRender.Subscribe(_ => onReady()));
}




public static class FagUtils
{
	public static Fag listen(this Fag tag, string eventName, Action action, bool condition = true) => tag.listen(eventName, null, _ => action(), condition);
